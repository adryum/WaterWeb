import data from "/data/map_data.json" with {type: "json"};

// Super important function for coords n stuff
...
let mapDataJson = data;

// Places markers in map
var map = L.map('map').setView([56.92, 24.1], 7);

L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
    attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
}).addTo(map);

// Adds all json info to the map as markers
mapDataJson['features']
    .forEach((entry) => {
        let usableCoords = LKS92WGS84.convertXYToLatLon(entry['geometry']['coordinates']);
        var marker = L.marker(usableCoords).addTo(map)
